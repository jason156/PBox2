...
二：使用方法
  Delphi 原工程文件，修改为 DLL 工程。输出特定函数就可以了。把编译后的 DLL 文件放置到 plugins 目录下就可以了。
  Delphi 函数声明：
    type
      { 支持的文件类型 }
  ...
  VC 原工程保持不变，编译得到 EXE。 新建一个 .CPP 文件，编译，和原来的编译 EXE 产生的 OBJ 文件，进行连接，得到 DLL 文件，放置到 plugins 目录下就可以了。
  VC 函数声明：
...
三：输出函数说明
  DLL 输出函数参数说明：
    ...
    strIconFileName     ：图标文件；
    bShow               ：是否显示；第一次调用 VC DLL 时，是不用创建创建 DLL 窗体的，只是为了获取参数。
  
  
四：特色功能
  界面支持，菜单方式显示、按钮（对话框）方式显示、列表视方式显示。
  PBox 还支持将一个 EXE 显示在窗体中。


五：接下来工作：
  添加数据库支持（由于本人对数据库不熟悉，所以开发较慢）
  添加 VC(MFC)/QT DLL 窗体的支持；